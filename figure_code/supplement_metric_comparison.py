...
save_name='Metric Comparison'
metric_name=['Alternative Precision Measure','Precision Measure']
...
metric[:,:,:3,1]=np.clip(metric[:,:,:3,1],a_min=-3,a_max=6)
metric1=metric[:,:,:,0]
metric2=metric[:,:,:,1]
cell=2
min_c1=np.round(metric1[:,:,3].min()).astype('int')
max_c1=np.round(metric1[:,:,3].max()).astype('int')
min_c2=np.round(metric2[:,:,3].min()).astype('int')
max_c2=np.round(metric2[:,:,3].max()).astype('int')

metric1=metric1.reshape(60,60,60,4,4)
metric2=metric2.reshape(60,60,60,4,4)
fig=plt.figure(figsize=(12,12),dpi=400)
gs0 = GridSpec(3,4, height_ratios=[1,2,2])
#fig2,axes=plt.subplots(2,4,height_ratios=[2,1])
#axes[0,0]=plt.subplot(2,3,3, projection='3d')
levels = np.linspace(min_c1, max_c1, 60)
color=plt.cm.bone(np.linspace(0,0.85,7))
dummy_scale=np.linspace(min_c1,max_c1,60)
dummy_scale=np.tile(dummy_scale,reps=(60,1))
levels2 = np.linspace(min_c2, max_c2, 60)
dummy_scale2=np.linspace(min_c2,max_c2,60)
dummy_scale2=np.tile(dummy_scale2,reps=(60,1))
for i,j in zip(range(4),['ksyn','koff','kon','max of three']):
    ax1 = fig.add_subplot(gs0[1,i],projection='3d')
    metric1[[5,17,29],-1,-1,cell,i]=min_c1
    metric1[[5,17,29],-1,-2,cell,i]=max_c1
    surface=ax1.contourf(X,Y,metric1[5,:,:,cell,i],zdir='z', offset=ksyn[5],cmap='viridis',levels=levels)
    ax1.contourf(X,Y,metric1[17,:,:,cell,i],zdir='z', offset=ksyn[17],cmap='viridis',levels=levels)
    ax1.contourf(X,Y,metric1[29,:,:,cell,i],zdir='z', offset=ksyn[29],cmap='viridis',levels=levels)
    ...
    if i==0:
        ax1.scatter([kon[10]] * 7,[koff[10]] * 7, ksyn[np.arange(5, 30, 4)], c=color[::-1], cmap='bone')
    elif i==1:
        ax1.scatter([kon[10]] * 7, koff[np.arange(10, 50, 6)],  [ksyn[29]] * 7, c=color, cmap='bone')
    elif i==2:
        ax1.scatter(kon[np.arange(10, 50, 6)], [koff[10]] * 7, [ksyn[29]] * 7, c=color, cmap='bone')
    if i==3:
        ...
        ax1.scatter(kon[np.arange(10, 50, 6)],koff[np.arange(10, 50, 6)], [ksyn[29]] * 7, c=color,cmap='bone')
...
surface=ax1.contourf(X,Y,dummy_scale,zdir='z', offset=ksyn[59],cmap='viridis')
surface.set_clim(min_c1, max_c1)
...
cax = plt.axes((0.9, 0.45, 0.02, 0.28))
cbar=fig.colorbar(surface,cax=cax,extend='both')
cbar.ax.set_ylim(min_c1,max_c1)
...
for i,j in zip(range(4),['ksyn','koff','kon','max of three']):
    ...
    metric2[[5,17,29],-1,-1,cell,i]=min_c2
    metric2[[5,17,29],-1,-2,cell,i]=max_c2
    surface=ax2.contourf(X,Y,metric2[5,:,:,cell,i],zdir='z', offset=ksyn[5],cmap='viridis',levels=levels2)
    ax2.contourf(X,Y,metric2[17,:,:,cell,i],zdir='z', offset=ksyn[17],cmap='viridis',levels=levels2)
    ax2.contourf(X,Y,metric2[29,:,:,cell,i],zdir='z', offset=ksyn[29],cmap='viridis',levels=levels2)
    ax2.set_zlim(ksyn[5],ksyn[29])
    ax2.view_init(azim=250)
    #cbar = fig.colorbar(surface, ax=ax1, extend='both')
    #cbar.ax.set_ylim(0, 1)
    if i==0:
        ax2.set_ylabel('koff in log10')
        ax2.set_xlabel('kon in log10')
        ax2.set_zlabel('ksyn in log10')
    ...
    #ax1.set_title('Metric of '+j)
    if i==0:
        ax2.scatter([kon[10]] * 7, [koff[10]] * 7, ksyn[np.arange(5, 30, 4)], c=color[::-1], cmap='bone')
    elif i==1:
        ax2.scatter([kon[10]] * 7, koff[np.arange(10, 50, 6)], [ksyn[29]] * 7, c=color, cmap='bone')
    elif i==2:
        ax2.scatter(kon[np.arange(10, 50, 6)], [koff[10]] * 7, [ksyn[29]] * 7, c=color, cmap='bone')
    if i==3:
        index=np.empty(shape=(3,0))
        #ax1.scatter([koff[10]]*7,[kon[10]]*7,ksyn[np.arange(5,30,4)],c=color[::-1],cmap='bone')
        index=np.hstack((index,np.array([range(5,30,4)[::-1],[10]*7,[10]*7])))
        ...
        #ax1.scatter([koff[10]] * 7, kon[np.arange(10, 50, 6)], [ksyn[29]] * 7, c=color,cmap='bone')
        index = np.hstack((index,np.array([[29] * 7, [10] * 7, range(10, 50, 6)])))
        ax2.scatter(kon[np.arange(10, 50, 6)], koff[np.arange(10, 50, 6)], [ksyn[29]] * 7, c=color,cmap='bone')
        index = np.hstack((index,np.array([[29] * 7, range(10, 50, 6), range(10, 50, 6)])))
        index=np.array(index,int)
        #c=ax1.pcolor(metric[29,:,:,cell,i],vmin=min_c,vmax=max_c)
        #fig.colorbar(c,ax=ax1)
surface=ax2.contourf(X,Y,dummy_scale2,zdir='z', offset=ksyn[59],cmap='viridis')
surface.set_clim(min_c2, max_c2)
ax2.set_xticks(ticks=[])
ax2.set_yticks(ticks=[])
ax2.set_zticks(ticks=[])
cax = plt.axes((0.9, 0.1, 0.02, 0.28))
cbar=fig.colorbar(surface,cax=cax,extend='both')
cbar.ax.set_ylim(min_c2,max_c2)
index=np.ravel_multi_index(index,dims=(60,60,60))
plt.rcParams["axes.prop_cycle"] = plt.cycler("color", color)
fig.text(0.05, 0.9, 'A', ha='center', fontsize=20,weight='bold')
fig.text(0.05, 0.65, 'B', ha='center', fontsize=20,weight='bold')
fig.text(0.05, 0.35, 'C', ha='center', fontsize=20,weight='bold')
fig.text(0.97, 0.1, metric_name[1], ha='center',rotation=90, fontsize=18)
fig.text(0.97, 0.45, metric_name[0], ha='center',rotation=90, fontsize=18)
fig.text(0.18, 0.05, 'ksyn', ha='center', fontsize=20)
fig.text(0.4, 0.05, 'koff', ha='center', fontsize=20)
fig.text(0.6, 0.05, 'kon', ha='center', fontsize=20)
fig.text(0.82, 0.05, 'max of three', ha='center', fontsize=20)
"""
